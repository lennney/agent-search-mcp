...
- Default offline suite: 720 passed; two live-network tests remain explicitly
  gated.
- A bounded release smoke used one DDG adapter attempt with no automatic
  retry and returned a non-empty result set through MCP stdio. This is a
  point-in-time regression check, not an availability measurement.
