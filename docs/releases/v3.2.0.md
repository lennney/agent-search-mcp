...
- The project-owned DuckDuckGo Web → HTML → Lite chain no longer depends on
  Python/ddgs.
- `free_search_news` now uses Bing News RSS directly, applies explicit
  24-hour/7-day/30-day publication windows, propagates cancellation, and
  reports provider failures instead of relabeling ordinary Web results.
...
- Its exact commit, SHA-256, packed contents, and six-cell matrix are retained
  in [the candidate evidence](../evidence/2026-07-26-release-candidate-a1de485.md).
- The HTTP/news fixes changed package source after that matrix. The retained
  tarball is historical evidence only; a publishable artifact must be rebuilt
  from the final PR commit and replayed across the same matrix.
- Default offline suite: 747 passed; two live-network tests remain explicitly
  gated.
