...
- CLI binary named `asm` to avoid overwriting MCP server entry
...
Usage:
  asm search <query> [options]    Search the web
  asm extract <url> [options]     Extract page content
  asm serve [options]             Start HTTP server
...
Examples:
  asm search "TypeScript MCP server"
  asm search "query" --count 5 --engines bing,baidu
  asm extract "https://example.com" --json
  asm serve --port 8080
`);
}
...
# JSON output
asm search "query" --json
```

### Extract Web Page

```bash
asm extract "https://example.com"
asm extract "https://example.com" --json
```
...
3. Binary name is `asm` (not `agent-search-mcp`)
...
- CLI binary named `asm` (short, memorable, avoids overwriting MCP entry)
