...
- CLI binary named `fas` to avoid overwriting MCP server entry
...
Usage:
  fas search <query> [options]    Search the web
  fas extract <url> [options]     Extract page content
  fas serve [options]             Start HTTP server
  fas --help                      Show this help
...
Examples:
  fas search "TypeScript MCP server"
  fas search "query" --count 5 --engines bing,baidu
  fas extract "https://example.com" --json
  fas serve --port 8080
`);
}
...
# JSON output
fas search "query" --json
```

### Extract Web Page

```bash
fas extract "https://example.com"
fas extract "https://example.com" --json
```
...
3. Binary name is `fas` (not `agent-search-mcp`)
...
- CLI binary named `fas` (short, memorable, avoids overwriting MCP entry)
