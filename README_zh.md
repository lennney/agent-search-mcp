...
| 使用显式代理 | `DUCKDUCKGO_PROXY_URL`、`SOGOU_PROXY_URL`、`MOJEEK_PROXY_URL`，或 `USE_PROXY=true` 配合 `PROXY_URL` |
| 使用用户自持代理池 | `DUCKDUCKGO_PROXY_URLS`、`SOGOU_PROXY_URLS` 或 `MOJEEK_PROXY_URLS`，JSON 数组 2-16 个 HTTP(S) 代理 URL |
